...
function process_invites(): void {
    ...
    foreach ($courses as $course) {
        ...
        if (!$questionnaire) {
            log::trace("❌ Nessun questionario nascosto trovato nel corso {$course->id}, salto.", $course->id);
            continue;
        }
        ...
        log::trace("✅ Questionario reso visibile per il corso {$course->fullname}", $course->id);
        ...
        log::trace("📅 Invio promemoria di apertura questionario per {$course->fullname} a " . count($incompleteusers) . " utenti", $course->id);
        
        foreach ($incompleteusers as $user) {
            log::trace("📧 Tentato invio messaggio a {$user->firstname} {$user->lastname} ({$user->email})", $course->id, $user->id, 'default');
